...
func (spp *sppImplementation) GetListSppForAdmin(meta *meta.Metadata) ([]model.GetListSppResponse, int, error) {
	...
	statement, params, err := spp.getlistSppForAdminQuery(notCount, q)
	if err != nil {
		log.Println(err)
	...
	// count total data
	statement, params, err = spp.getlistSppForAdminQuery(count, q)
	if err != nil {
		log.Println(err)
		return nil, 0, errors.New("build statement query to get spp from database")
	}
...
func (spp *sppImplementation) getlistSppForAdminQuery(is_count bool, q *param.Query) (string, []interface{}, error) {
