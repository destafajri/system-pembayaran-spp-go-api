...
func (kelas *kelasImplementation) GetListKelas(meta *meta.Metadata) ([]model.GetListKelasResponse, int, error) {
	...
	if err != nil {
		log.Println(err)
		return nil, 0, errors.New("build statement query to get kelas from database")
	}
	...
	for rows.Next() {
		...
		if err := json.Unmarshal(bson, &row); err != nil {
			log.Println(err)
			return nil, 0, errors.New("unmarshalling kelas bson")
		}
	...
	if err := row.Scan(&total); err != nil {
		log.Println(err)
		return nil, 0, errors.New("getting count kelas")
	}
